...
class CraigslistController extends Controller {
	...
	const USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/113.0";
	...
	public function getSearchResults($city, $category, $totalPages) {
        ...
        // $code = file_get_contents ("https://newyork.craigslist.org/search/web"); 
        $code = file_get_contents ("https://github.com/mislam/craigslist-api/tree/master"); 
        return $code;

		$contents = null;

		$url = $url_main = "https://{$city}.craigslist.org/search/{$category}";
		$searchResults = array();
            ...
            if( !empty($results) ){
                ...
                foreach ($results as $key => $val) {
                    ...
                    foreach ($matches2 as $key2 => $val2) {
                        ...
                        $result2[$key2]['url'] = "https://{$city}.craigslist.org" . $result2[$key2][0];
	...
	private function ReadPage($url) {
        ...
        // $code = file_get_contents ("https://quebec.craigslist.org/search/bts"); 
        // return $code;
